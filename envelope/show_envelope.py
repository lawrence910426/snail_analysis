...
def show_curve(upper_i, upper_v, lower_i, lower_v, rawx, rawy, tag):
    ...
    return [[np.exp(beta_up), alpha_up], [np.exp(beta_low), alpha_low]]
...
def show_envelope(file_name, x, y, config):
    ...
    return show_curve(a, b, c, d, x, y, file_name if config["show_envelope"]["show"] else "")
